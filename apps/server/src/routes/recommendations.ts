...
import { recommendationRequestSchema } from '@nutritime/contracts';
 ...
 *
 * Reasons are taken in descending points with the declaration order of `SCORE_REASON_KINDS` as
 * the tie-break, so the same meal always produces the same sentence.
 */
export function fallbackExplanation(scored: ScoredMeal): string {
  const cited = scored.scoreReasons
    .map((reason, index) => ({ reason, index }))
    .filter((entry) => entry.reason.points > 0)
    .sort((left, right) => right.reason.points - left.reason.points || left.index - right.index)
    .slice(0, MAX_CITED_REASONS)
    .map((entry) => entry.reason.detail);

  if (cited.length === 0) {
    // Every policy scored zero. Saying so is better than inventing a reason: the meal is
    // eligible, which is the only claim the data supports.
    return 'This one fits your preferences.';
  }
  const [first, ...rest] = cited;
  const head = first ?? '';
  const tail = rest.map((detail) => detail.charAt(0).toLowerCase() + detail.slice(1));
  const joined =
    tail.length === 0
      ? head
      : tail.length === 1
