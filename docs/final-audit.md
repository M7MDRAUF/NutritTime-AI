...
| `npm run test`                            | **123 files · 3254 passed · 1 expected fail · 6 skipped** — was 119 / 3142 / 1 / 2 at `dc4eca5`; the four extra skips are the real-model cases, gated on `RUN_MODEL_TESTS`                                                                                                                                                       |
...
| `npm run test:e2e`                        | **283 passed**, four viewport projects, exit 0, no flaky — was 276                                                                                                                                                                                                                                                               |
| `Plan.md` §17.1                           | **63 rows · 63 files · 0 drifted · 0 missing · 0 dead** — was 57/57. The reconciler itself had a third blind spot: it walked only `apps`, `packages` and `e2e`, so `eslint.config.mjs` (root) and `.github/workflows/ci.yml` both reported DEAD while being over the cap. It now scans the whole repository and excludes by name |
...
**The eleven Partial rows, each with its gap — as this audit found them, followed by what the
closing wave did to each.** The original text is kept rather than replaced: four of these claims
are now false, and a verdict that silently rewrites its own reasoning cannot be audited twice.

| Row     | The gap this audit found                                                                                                    | Where it stands after the closing wave                                                                                                                                                                                                                                                                                                                                                                                 |
| ------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T-22-03 | 9 of 10 deep-link paths; `/splash` is a free choice, not an impossibility                                                   | **Completed.** Restoring `/splash` was measured and it **strands** the user — zero interactive elements, no tab bar, nothing to advance it — so `Splash` left the linkable set and the row is **9 of 9**. The tenth case is now a degradation assertion that reddens if anyone puts it back                                                                                                                            |
| T-22-08 | bundle measured, lazy image loading not built                                                                               | **Still Partial, and one half is unmeetable.** Favourite rows no longer all build at once (`20 of 20` → `9 of 20`, `20 <img>` → `9 <img>`), but `loading="lazy"` **cannot be expressed in react-native-web 0.21.2** at all                                                                                                                                                                                             |
| T-23-01 | role/label audit — **its gap is named nowhere and its "Audit table" evidence does not exist**; the hollowest row in the set | **Completed, and BOTH clauses above are now false.** 231 interactive instances over **18 real surfaces**, 117 distinct role+id+name triples, **0 without a role, 0 without a name**; `docs/accessibility-roles.md` is the table. Emptying the enumeration left all 18 per-surface tests **green** — only the count assertions caught it, so the bookkeeping is the test                                                |
| T-23-03 | no screen rendered above `fontScale 1`, no screenshots                                                                      | **Still Partial, and now with a reason rather than an absence.** 10 screens × 4 viewports, 40 screenshots, and text **does** clip at 2× (all five tab labels, `needed 27 / given 16`, at 320/375/414 and not at 768). But the app **cannot receive a scale on the web**: react-native-web pins `fontScale` to 1 and `Dimensions.set` throws, so the scale is simulated and `isLargeText` is dead code there (**R-83**) |
| T-23-05 | focus half built on the meal form only                                                                                      | Still Partial — met on that form for both halves it names; the second form is not covered, and `FormField` makes **every** field error assertive                                                                                                                                                                                                                                                                       |
| T-23-07 | the tool's pre-delivery checklist is not evidenced "in full"                                                                | Still Partial. The contrast half was already met; the checklist half is unchanged                                                                                                                                                                                                                                                                                                                                      |
| T-24-03 | Explore excludes nothing — see part 6                                                                                       | **Still Partial, and the gap moved.** FR-007's screen-level disclaimer is now on Explore, and the ruling behind it is that PRD scopes allergen rejection to _recommendations_ while TSD §8.4's "out of Explore" contradicts three PRD clauses. What is unbuilt is the **row-level** notice, and a `MealCard` props change is a TSD §6.7 stop condition. The disclaimer is also what caused **R-82**                    |
| T-25-02 | search and filtering are not local, so the row does not describe this architecture                                          | **Still Partial, and now registered as X-50.** Confirmed by measurement: the chip's own state flips in **11.4 ms** while chip → rows painted is **30.8 ms**, and a 250 ms delay injected on the wire moves the first by **−0.3 ms** and the second to **289.8 ms** in the same session                                                                                                                                 |
| T-25-04 | warm chat latency unmeasurable without a model                                                                              | **Completed — measured against a real `gemma3:4b`.** Median **5335.5 ms**, worst **5480.5 ms** against ~11 s soft / 30 s hard; the 30 s timeout fired four times and ended each request **at** the budget as 503 `ai_unavailable`                                                                                                                                                                                      |
| T-25-05 | 3 of PRD §10.1's 4 cold-start claims                                                                                        | **Still Partial, and WORSE than unmeasured (R-79).** "Roughly a minute" is confirmed (43 767 ms load), but **"fail and then recover on its own" did not occur once in four runs**: the 30 s abort cancels the in-flight load, so after a machine restart **the assistant is 503 forever** and no request the app can issue is long enough                                                                              |
| T-26-01 | no Run URL can exist                                                                                                        | Still Partial, and still for the same reason: there is no remote, so no Run URL exists and none was fabricated                                                                                                                                                                                                                                                                                                         |
