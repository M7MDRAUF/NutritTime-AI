...
| X-26 | **PRD §10.4 says files stay "under 350 lines"; SQG-09 and T-28-05 say "≤ 350".** **five files are exactly 350**, so each is a violation under one document and compliant under the other: `apps/mobile/src/infrastructure/storage/definitions.ts`, `apps/mobile/src/shared/components/AccessibleButton.dom.test.tsx`, `apps/server/src/ai/__fixtures__/replies.ts`, `apps/server/src/ai/claimDenylist.test.ts` and `e2e/specs/favorite-persists.spec.ts`. **The row named only the first until the P28 audit counted them**, and five MORE sit at 349, so the decision moves ten files one way or the other | PRD §10.4 vs Plan SQG-09 / T-28-05 | Whether T-28-05's automated re-check fails the build at P28 | **OPEN.** PRD outranks Plan, which would make it a violation needing a §17.1 row — but a one-word difference across two documents is more likely an editorial slip than an intent, and it affects every file the project will ever write. A one-line edit to whichever is wrong settles it | P28 |
...
| X-47 | **The error summary on `DietarySetupScreen` is unbuilt — and this row's original premise, that the two forms differ on a FOCUS MOVE, was false.** It claimed `MealFormScreen.tsx` had "summary + focus move" and `DietarySetupScreen.tsx` neither. Measured at P28: **`.focus(` has ZERO occurrences in any production file under `apps/mobile/src`** — it appears only in four test files, and `MealForm.dom.test.tsx:746` names a `.focus()` on the summary as the mutation it probes **against**, because it "would have dumped the focused box". So **neither** form moves focus, `Plan.md`'s "Preserved on validation failure" is honoured everywhere, and the difference between the two forms is the **summary alone** | Certain, measured by search; `design-system/DECISIONS.md` contradicts itself on it (two sections say "both", one says summary only) | A screen-reader user on the onboarding form gets four assertive alerts and no lede; the meal form gets a summary | **What changes because the premise was false: the summary half needs NO user decision.** It is an adopted design recommendation, implemented on one of two forms, forbidden by nothing — a commit, not a ruling. The agent that raised a stop condition here was right to (it had been told the pair was implemented next door) and the coordinator supplied the false half by relaying another report's claim without measuring it — BRIEF §6.1p, third instance. **What the user still decides, and it is smaller than this row first said:** whether a summary should ever TAKE focus, which the Plan's "preserved on validation failure" reads against and which no form currently does | Accessibility Engineer | **Summary: a commit. Focus move: user decision** |
| X-48 | **Two rows of `PRD.md` §10.1 cannot both hold under the shipped default, and the figure that looks like a PASS was measured with no model.** §10.1 asks for "App start to usable Home | **≤ 2.5 s** after first launch" and "Recommendation explanation, warm model | **~5 s**, hard timeout 12 s". `apps/server/src/routes/recommendations.ts:281-286` **awaits `explain()` for each of the three selected meals, sequentially, before `response.json(body)`** — so the three cards cannot reach the device until every explanation has finished or the shared 12 s deadline has expired — and `apps/mobile/src/infrastructure/storage/definitions.ts:276` ships `aiEnabled: true`, so that is the DEFAULT path, not an opt-in | **MEASURED at P28 against a real `gemma3:4b`, and every clause of the prediction held.** Per explanation **4878–6265 ms** — §10.1's "~5 s" is accurate. The server leg is **10 429–12 118 ms** against the ≤ 2.5 s row: a **4.2–4.8× MISS on the default path**, with a control of **19.4 ms** with AI off, which is the 204.1 ms figure's real meaning. **And one thing the prediction did not reach: three explanations do not merely exceed the 12 s deadline, the third is CUT — in 4 of 6 runs** — so the shipped default degrades the third card silently rather than failing visibly. (Previously: "Certain with a real model; invisible without one", and it was invisible for nine phases because no model existed here) | The first screen every user sees on first launch | **Recorded, not resolved, and the target is NOT relaxed** — P25 Part 5: "a missed target is recorded as a known limitation, never by relaxing the target". **The ambiguity is what "usable Home" means**, and the two readings give opposite verdicts: if it means the screen is interactive and showing its computed meal period — which P15 renders **before any request**, deliberately, so the screen is never blank while it waits — then ≤ 2.5 s holds and the cards arriving later is the design. If it means the three recommendations are readable, the default configuration cannot meet it with a model running. **What the user decides:** which reading §10.1 intends, and if the stricter one, whether the fix is a streamed/two-stage response (cards with fallback text first, upgraded when the model answers), parallel explanation calls, or a different default for `aiEnabled`. Note the P25 figure should be read as "with AI unavailable" whichever way this is ruled — and it now has a measured companion that says so numerically (19.4 ms off, 10 429–12 118 ms on). **The decision is unchanged and still the user's; what changed is that it is no longer a prediction** | Performance Engineer | **User decision** |
| X-49 | **TSD 5.7 specifies "four checks" and the shipped behaviour applies five.** The fifth is the P24 citation guard at `apps/server/src/routes/chat.ts:273-276`, which discards a reply whose rendered `citations` would be empty while `resolved.namedMeals` is not. It was put in the route **specifically to keep `containReply` at four**, and an independent audit ruled that this does not help: the count a reader cares about is how many discards a reply can suffer, and that is now five wherever the fifth lives | `TSD.md` 5.7 vs `apps/server/src/routes/chat.ts` — and **PRD 7.3 vs TSD 5.7**, which is the part that settles it | Nothing behavioural; the guard is correct and was independently ruled **faithful**, with `ai_unavailable` "the only defensible code of the five" | **The code is right and the document is now incomplete.** PRD 7.3 — "Show the meals the answer drew on as citations" — compels the discard and **outranks** TSD. The guard is pinned in effect *and* in shape (the probe that re-keyed it to the model's own `answered` flag reddens the discriminating test). **But it also MASKS check 1 at the route layer, measured:** with check 1 dead *alone* the route suites stay green, because the guard yields the same 503 — only killing both reddens (12 fail). Nothing is unguarded (the guard's condition is provably equivalent to `citedMealIds` being empty downstream of check 1), but the route suite is no longer a discriminating oracle for check 1 and the completion lives in another file with no structural link. That is an argument for the fifth check living in `containReply` after all. Recorded rather than resolved because a document is never edited to match code. **What the user decides:** whether TSD 5.7 is amended to describe five checks and name where the fifth lives, or whether the fifth is moved into `containReply` so the document's count becomes true again — the second is a refactor with no behavioural change | AI Engineer | **User decision** |
| X-50 | **`PRD.md` §10.1's "Local navigation, search, filtering ≤ 150 ms over the meals already loaded" describes a local catalogue the app does not implement.** `apps/mobile/src/features/catalog/exploreFilters.ts` states the opposite in its own header — "No filtering logic lives here. The chips choose parameters; the server filters, through the domain" — and `useMealSearch.ts` debounces the text and then issues one `client.listMeals` per settled query, so **every search and every filter is a round trip**. PRD FR-010's "Results page locally" is implemented the same way. SDD §11, the lower document, already describes the design that contradicts the higher one | **Certain**, read from both files and measured: the chip's own `aria-checked` flip is **11.4 ms** while chip → rows painted is **30.8 ms**, and a 250 ms delay injected on `/api/v1/meals` moves the first by **−0.3 ms** and the second to **289.8 ms** in the same browser session | T-25-02 cannot be given a verdict as written — only navigation and the local sub-boundaries can, and all of those pass by an order of magnitude | **Recorded, not fixed — PRD outranks everything here. What the user decides:** whether §10.1 means the app should hold the catalogue and narrow it on the device — a real architectural change, and one TSD §4.7's "one implementation, two callers" rule argues against, since a screen narrowing its own list would be a third source of relevance — or whether the row should state what the build does and set a separate budget for a server-backed query. **Relaxing 150 ms is forbidden either way** (P25 Part 5); saying which reading is intended is not | Performance Engineer | **User decision** |
...
| Web idioms: `cursor-pointer`, hover states | **Rejected for native; partially applicable to the web export** | RN has no hover. On the web surface, hover is an enhancement only. **The governing rule is this document's own §20 row — *"No hover-only interaction / Every hover affordance has a tap/focus equivalent"* — and NOT PRD §10.5**, which this cell cited until P28: `PRD.md` contains the word "hover" **zero** times, and §10.5's four bullets are roles-and-labels-and-touch-targets, text scaling, colour independence, and AA contrast. It is one of **three** false PRD §10.5 citations the P28 design-system audit found — the others were for **focus** and **tab order**, words `PRD.md` also does not contain — and it is the same mechanism as the `PRD §15.5` mis-citation that once reached 18 files: a plausible section number, cited with confidence, never opened |
...
SQG-09 caps a new file at 350 lines. **Fifty-seven files exceed it.**

**What "file" counts as, because the question has a real answer and two open edges.** Counted:
`.ts`, `.tsx`, `.mts`, `.mjs`, `.js`, `.yml`, `.yaml` anywhere in the repository. `.md` is NOT
counted — SQG-09 reads "Every new **file** ≤ 350 lines — PRD §10.4 says files, not only
components", and PRD §10.4 sits in Maintainability beside "Strict TypeScript" and "Domain logic
lives in the domain package"; **no row in this table names a `.md` file** — asserted by the reconciler rather than tallied in prose, because a count in a sentence is how "57" survived four rows being added — while this document alone
is over 3000 lines. Recorded as a judgement where the documents are silent.

**Two edges are NOT settled and are the user's to settle.** (1) `packages/catalog/meals.json`
(**5621** lines) and `packages/catalog/nutrition-source.json` (**1488**) are inside T-28-05's own
`packages/**` scope and have no rows. Both are **generated** — `npm run seed` writes them, which
is why `.prettierignore` names them — and **no document defines a generated-file exclusion**, so
they are reported rather than counted. (2) T-28-05's own task-list row still says "every
**component** ≤ 350" while its acceptance row says "every **file** … not components only"; under
the narrower wording **every test file, every workflow and config, and every non-component module in this table falls out of scope — the majority of it**. Stated as classes rather than as a tally on purpose: the tally was "32 of 57", it required arithmetic no reader can redo from the table, and it was wrong the moment a row was added. The acceptance is the stricter and
later text and is what this table is measured against.

**Reconciled by script, not by hand** — `T-28-05`'s job made repeatable. Two bugs in that script
were themselves found by the P28 audit and by its own first run against a dot-directory: an
extension filter that omitted `.yml` (and so LOST a row its author had filed for
`.github/workflows/ci.yml`), and `lstrip('./')`, which strips a character *set* and turned
`./.github/…` into `github/…`. A reconciliation tool that silently skips files is the same defect
as the table it exists to check. Each is approved here with its
...
| `packages/domain/src/scoring.test.ts` | 515 | Test file: one band-edge table per policy |
...
| `eslint.config.mjs` | 457 | **Crossed the cap at P26.** It is one flat config: a single ordered array whose later entries deliberately narrow earlier ones, and ESLint **replaces** rather than merges `no-restricted-imports` for overlapping globs — so the order is behaviour and a reviewer has to read it whole. **Four** of its blocks restate `no-console` minus a clause rather than switching it `off` — the mistake P01 learned — over a project-wide baseline that allows only `warn` and `error`. **This row said "five … five times" and both figures were wrong**: there were three restatements and one actual `off` for test files, which is the very mistake the comments around it forbade in bold. P28 converted the `off` to a restatement, making four, and **deleted the running tallies from the file rather than renumbering them** — a count in a comment is a claim that rots, and `grep -n no-console` is the answer that cannot; splitting it would put a restatement in a different file from the rule it narrows. Crossed 350 when `scripts/*.mjs` gained the block `e2e/serveExport.mjs` already had, for the dev-server launcher that replaced a `&` which does not background on Windows (X-46) |
| `apps/mobile/src/navigation/TabNavigator.tsx` | 383 | **Crossed the cap at P28, closing R-72.** It is the five-tab bar and everything the bar decides: the icon set, the two tint slots measured against `surface.raised` in both schemes, `backBehavior="history"` with the measurement of all six library behaviours behind it, `lastTab` persistence, and now the bar height that makes the labels legible. Splitting is not available — `navigation/**` is spine and the pieces are one screen's chrome, so a reviewer needs the tint, the height and the label metrics together: the R-72 fix is only comprehensible beside the library arithmetic it works around (`49 − 1 − 10 − 28 = 10` px for a label needing 14). |
| `.github/workflows/ci.yml` | 388 | **Added at P26.** One job whose steps are ordered and interdependent — `npm ci` → `check` → both builds → E2E with `AI_FAKE` → audit — so splitting it into reusable workflows would scatter the sequence a reviewer has to read whole, and a reusable-workflow boundary is also a second place for the step order to drift. Most of the length is R-59's guard, which exits 1 on a missing `USDA_DATASET_PATH` with an `::error` naming the variable, the file and the seven tests it would otherwise skip silently — exercised in five states (present, absent, `optional`, bad path, both unset). **This row was filed by the agent that wrote the file and then LOST by the §17.1 reconciliation**, whose extension filter covered only `.ts/.tsx/.mts/.mjs/.js`; the P28 audit found it. |
| `apps/mobile/src/shared/components/roles.dom.test.tsx` | 512 | **Added at P28 — T-23-01's whole evidence base.** It mounts **18 real surfaces** of the app — the ten registered screens, five second states, the tab navigator, the error boundary and the shared components no screen reaches — behind one provider spine, and asserts a role **and** an accessible name on every interactive element each one renders: **231 instances, 117 distinct role+id+name triples, 0 without a role, 0 without a name.** The definition of "interactive", its exclusions **with their reasons**, and the closure assertion that stops the exclusion list widening are the file's substance rather than its preamble — emptying the enumeration left all 18 per-surface name tests **green**, and only the two count assertions caught it, so the bookkeeping IS the test. Trimmed from 618; further cuts would come out of the stated definition, which is the part the task calls load-bearing |
| `apps/mobile/src/features/catalog/ExploreScreen.tsx` | 388 | **Crossed the cap at P28, closing R-74's second door.** It is Explore end to end: the deep-link seed read through the route table's own readers and now bounded at `MEAL_QUERY_MAX_LENGTH`, the search field, three filter chip groups, the paged list with its stable keys and memoised rows, and the loading, empty, local-only and error states. Splitting is not available on a seam that exists: the data hook is **already** extracted (`useMealSearch.ts`) and the filter shaping with it (`exploreFilters.ts`), so what is left is one screen's own markup and the param reading that decides its initial state — and the reason the length bound lives beside the array refusal is that they are the same decision about the same URL, made oppositely on purpose |
| `apps/mobile/src/features/catalog/useMealSearch.ts` | 372 | **Crossed the cap at P28 fixing F-6.** Explore's data hook, and the count is almost all reasoning: debounce, stale-request abort, generation counting, the three states, paging, and the delay rule that was **stated correctly and implemented wrongly for two phases** — keyed off whether the box was empty rather than off what the user had just done, so a chip tap paid the full 300 ms (**30.8 ms empty vs 333.9 ms with text**, on a 150 ms target). The false sentence is kept visible above the fix. It cannot usefully be split: the abort is the reason the hook exists at all, and a reader who cannot see the timer, the generation counter and the abort together cannot check any of the three |
| `e2e/specs/font-scaling.spec.ts` | 447 | **Added at P28 for T-23-03.** Ten screens (`PRD.md` §11) × four viewports, each **tapped rather than URL'd**, with 40 screenshots and a `font-scaling.json` per test. The count is the coverage: every screen needs its own route in, its own settle condition and its own container assertion, and the file additionally carries the reason the scale has to be injected at all — react-native-web pins `fontScale` to 1, so the app cannot receive one (R-83). It cannot be split by screen without ten copies of the observable, and the observable is the part worth reading once |
| `e2e/specs/viewport.spec.ts` | 411 | **Crossed the cap at P28, adding R-82's marker.** The layout suite for all four supported widths, and most of the count is the OBSERVABLE rather than the cases: `body.scrollWidth > body.clientWidth` is false while the defect is present, so the file walks each box's ancestors for the nearest horizontal clip and compares against that, and it records why two simpler observables were rejected — one of which passes under a button sitting 216 px outside a 320 px viewport. It also carries its own sensitivity test, which widens a real control at load time to prove the assertion can fail. Splitting by screen would copy that observable into each piece, and the observable is the part that has to be read once and trusted |
...
| T-22-03 | Direct-URL navigation into every deep-linkable screen | T-22-02 | Completed |
...
| ID | Task | Depends | Status |
|---|---|---|---|
| T-23-01 | Role and label audit across all interactive elements | T-18-07, T-21-07 | Completed |
...
| T-23-05 | Focus preserved on validation failure; async results announced | T-23-01 | Partial |
...
| T-24-03 | Spec 2 — peanut allergy excludes meals from Home and Explore | T-24-01 | Partial |
...
| T-25-04 | Measure warm chat latency and confirm the 30 s timeout behaviour | T-24-07 | Completed |
...
| ID | Task | Depends | Status |
|---|---|---|---|
| T-28-01 | Requirement coverage audit — all 15 FRs against implementation and tests | T-27-05 | Partial |
| T-28-02 | Contract audit — all five endpoints against §11 | T-28-01 | Completed |
| T-28-03 | Architecture audit — the six dependency rules; no cycles; no duplicated domain logic | T-28-01 | Partial |
| T-28-04 | Code hygiene — dead code, unused imports, orphaned files, stale comments, debug logs, `any`, unsafe casts, suppressions | T-28-03 | Completed |
| T-28-05 | Naming and size — PascalCase components, camelCase functions, kebab-case ids, every component ≤ 350 lines | T-28-04 | Completed |
| T-28-06 | AI audit — prompts, schemas, four containment checks, fallback behaviour, no leaked user data in logs | T-28-02 | Completed |
| T-28-07 | Design-system compliance — tokens used, no colour literals, both themes, tool checklist | T-28-05, T-23-07 | Partial |
| T-28-08 | Mobile-web and accessibility audit against §20 and §23 | T-28-07, T-22-08 | Partial |
| T-28-09 | Full evidence run: typecheck, lint, format, all test tiers, audit, both production builds, E2E | T-28-06 | Partial |
| T-28-10 | **Write the final audit report and issue GO / CONDITIONAL GO / NO-GO with evidence** | T-28-07, T-28-08, T-28-09 | Completed |
...
| SQG-09 | Every new **file** ≤ 350 lines — PRD §10.4 says files, not only components; an exception is documented and approved. **Fifty-seven approved exceptions are recorded in §17.1 below**; every one is a single cohesive table or one exhaustive `switch`, where splitting would scatter a thing a reviewer needs to read whole |
...
| R-55 | **A chip group conveys no selected state on the web build.** **FIXED at P23, and this Risk text is now false for a THIRD reason** (found at P28): it says `ChipRow` renders a `radiogroup` containing `button`s, and `apps/mobile/src/features/onboarding/ChipRow.tsx:66` renders `accessibilityRole="toolbar"` with toggles and a group label — the repair changed exactly this, and the row was never marked closed. The original defect: `aria-selected` is invalid on `button` and `accessibilityState` was unmapped by react-native-web 0.21.2, both verified in `node_modules`. The group also carries no accessible name | Certain on web | Medium — a screen-reader or colour-blind user cannot tell which diet, goal or budget is selected | Affects **three groups on one screen** — diet, goal and budget in `DietarySetupScreen` (P14). **The row previously said "four groups across two screens", counting the Settings theme row; that was wrong and self-contradictory**, because the next sentence describes Settings as already working around it. Re-measured at P23. Settings works around it locally — chips rendered directly, with the group name and state folded into each accessible name **plus** the selection in visible text — and says at the site why it does not use `ChipRow`. **The stated reason for deferring this to a phase was also false.** The row claimed the shared fix "changes accessible names that `DietarySetup.dom.test.tsx` asserts" — measured at P23, that suite asserted **nothing** about the selected state: deleting `selected` outright left **15 passed / 0 failed**. So the property was both invisible to the user and unpinned by the suite, which is how it survived nine phases. It was still right to give it a gate, for the opposite reason to the one recorded: there was no assertion to break, and therefore nothing that would have told anyone if the fix were wrong | Design-System Author | **P22** |
| R-56 | **`IconButton` publishes no PROP for an `accessibilityState`** — and the wording is load-bearing: `apps/mobile/src/shared/components/IconButton.tsx:54` **does** publish `accessibilityState={{ disabled }}`, hardcoded, so what is missing is the prop, not the attribute. `docs/phase-reports/P23.md` compressed this to "publishes no `accessibilityState`", which is **false**; the row is right and the paraphrase dropped the words that made it so, so the favourite toggle announces its action but carries no toggle *role* semantics | Certain | Low-to-medium — the state is conveyed, just not as a toggle | TSD 6.7 **fixes `IconButton`'s props**, so a **sixth** — `TSD.md` §6.7 lists `{ icon; accessibilityLabel; onPress; disabled?; size? }`, five props, so "seventeenth" was a category error importing §6.7's sixteen-**component** count into a **prop** count (one auditor read it as a seventh; re-counted from the row here) prop is an amendment rather than an edit. Same class as R-47 and R-48, which P14 recorded rather than faking with a cast. The interim satisfies PRD 10.5 in three ways that work on every platform: the heart/heart-outline **shape**, an accessible **name** that changes with state, and a visible caption — all asserted in both directions. The exact edit is parked in the P16 verification report for whoever amends 6.7 | Design-System Author | **User decision** |
...
| R-62 | **Three of the documents' own coverage claims are false in named places.** TSD 9 states that every module in 4-7 appears in its table — it omits `text`, `money`, `diet`, both allergen lexicons and all of `packages/catalog`, all six live and tested; three test files it names do not exist. **§11.2 names `health.integration.test.ts`, which does not exist** — those tests are at `apps/server/src/boot.integration.test.ts:87-107`. **And §22's traceability matrix is wrong in four named places, found at P28** — the matrix T-28-01 consumes: FR-003's row cites only `preferencesState.test.ts`, and deleting the discard at `useRecommendations.ts:191` leaves that file **55/55 green** while the requirement is actually pinned by `Home.dom.test.tsx:833`, which §22 cites for nothing; FR-009 names `explanation.integration.test.ts` and FR-014 names `settings.dom.test.tsx`, **neither of which exists** — and the second differs from the real `Settings.dom.test.tsx` only in case, so it resolves on this machine and **fails on a case-sensitive runner**; and FR-001's "satisfied vacuously" note is **stale**, since `hydrateStorage` has a caller at `StorageProvider.tsx:67`. Plan 19.5's evidence row `npm run test -- a11y` matches no file, and it appears **twice**, not once — in P23's "**Part 4.** `npm run test -- a11y` · the contrast test · manual screen-reader traversal" and again in §19.5's "Accessibility checks" row — so a reader who corrects one still meets the other. **This clause carried two LINE numbers until P28 and both were wrong**, by +12, because register rows kept being inserted above them: the correction of a false row was itself false, in the manner BRIEF §6.1q forbids by name. Quoted instead, which does not expire. **The working command is** `npx vitest run apps/mobile/src/shared/theme/{contrast,component-contrast,navigation-contrast,contrast-exemptions,touch-target,typography}.test.ts` — **6 files / 476 passed**. P23 Part 4 names the same command, which means **that phase cannot run its own Part 4 as written**. Plan 18's P17 command `npm run test -- saved-meals customMeals` has a `saved-meals` half that matches nothing, so it runs the store suite and none of the **seven** `features/saved` suites (the row said six; re-counted at P27) | Certain | Medium — a traceability table that is believed and wrong is worse than none, and T-28-01 consumes it | Recorded, not reconciled: a document is never edited to match code. Each is a one-line correction to whichever document is wrong, and **which one is wrong is the user's call** — the tree is right about the modules, and the commands are simply mistyped | Planner | **User decision** |
...
| R-67 | **The prompt shows the model four figures per meal that the answer is then forbidden to quote.** TSD 5.6 fixes the ten meal-block fields, four of them numeric (prep minutes, price, calories, protein). TSD 5.7's permitted set is `resolved.figures` plus digits in meal NAMES, and TSD 4.9 keeps `figures` deliberately narrow ("superlative → the winner's figure only"). So a model that adds a **true** fact read from its own context quotes an unpermitted figure and the whole reply is discarded as a 503 | **MEASURED at P28 against a real `gemma3:4b`, and this row was right about the rate and wrong about the rule.** Refusals ran at **3 of 14 (21.4%)** across two sessions — an elevated rate, as predicted — but **check 3 (`ungrounded-figure`) fired zero times** and every discard was **check 1 (`uncited-meal`)**. The embellished-figure mechanism this row describes was not observed at all; TSD §5.7's own four-of-six wrong-number defect is **not reproducible** against the shipped prompt, and RULES clause 7 steered the model on 8 of 8 safety baits. The real cause of all three refusals is **R-81** | Medium, and the shape is not the one predicted: the loss is a correct **count** answer discarded for an invented citation, not a true nutrition figure discarded for being unpermitted | **Correct as specified and not changed.** The domain decides which numbers are assertable; widening the permitted set to every figure in the prompt would let the model assert any nutrition number about any shown meal, which is most of what check 3 is for. Discarding an embellished reply loses fluency, not truth. This is the strongest argument for keeping the `RUN_MODEL_TESTS=1` probe alive — the refusal rate is measurable only against a model, and Plan 19.6 should register it. **That argument is now vindicated and spent:** the probe is what produced the measurement above, and what it found was that this row named the wrong check. **A risk row is a hypothesis with a date on it** — the fourth in this project found to carry false text once measured, after R-44's symptoms, R-55's justification and R-74's "no document answers" | AI Integration Architect | **Closed as a prediction; the live risk is R-81** |
...
| R-70 | **Containment's threat model covered the model and not the server.** Every guard — the four checks, the denylist, the figure extraction — runs on a MODEL reply. Nothing ran on the server's own user-facing copy, and `ai/chatCopy.ts`'s three strings reach a user verbatim | Was reachable and measured: rewriting `greeting` to "Hello. Every meal I can show you is allergen free." failed **0 of 824 tests** | **Was CRITICAL.** A peanut-allergic user asking "what can you do?" would have read that every meal is allergen free. And `chat.integration.test.ts:438` asserts that a **model** emitting that same phrase is discarded with a 503 — so the identical sentence was forbidden from the model and permitted from us | **CLOSED at P28, both halves — and the asymmetry the audit found was this.** The MOBILE sweep in `apps/mobile/src/features/assistant/assistantCopy.test.ts` was changed at P23 from a hand-written list to a **walk** of the copy objects, so a new string there is swept by construction (measured: the same mutation reddens). **The SERVER sweep, where this risk originated, is still a hand-written list of three** at `apps/server/src/ai/chatCopy.test.ts`. All three of `CHAT_COPY`'s current keys are on it, so **nothing escapes today** — but a **fourth** key is opt-in: adding one carrying verbatim *"Every meal I can show you is allergen free."* fails **0 of 866** server tests, with the mutation proved loaded rather than inert. **Both halves are now closed (P28 remediation).** The server sweep was converted to a walk rooted at the **module namespace**, so a newly *exported constant* is swept too, not merely a new key — and the counterfactual was measured rather than asserted: under a load-time fourth key carrying *"Every meal I can show you is allergen free."* the new walk fails **1 of 25** and names the key, while the **old hand-list passes 14 of 14** with an appended witness proving the key was loaded and the mutation therefore live (§6.1k). Three further controls redden: a dead walk (**5** failures, including `expected 8 to be greater than 8`), a digit in copy (**1**), and a neutered `deniedClaimIn` (**2**). **The original closure text follows and was true as fact and false as structure:** `ai/chatCopy.test.ts` ran every user-facing string, and every `copyForUnresolved` reason, through **`deniedClaimIn` itself** — the same function containment uses, an independent authority covering all 22 phrases by construction rather than a transcribed subset. Reason-list completeness is a compile error via `satisfies Record<UnresolvedReason, true>`. A test rather than a runtime check, because the copy is a compile-time constant. **The general lesson is recorded rather than the instance: a guard aimed at one source of text does not cover another**, and P21's mobile half adds more user-facing copy | AI Integration Architect | P23 / P28 re-check |
| R-71 | **A resolved answer whose model reply omits citations is discarded as a 503, and the domain could have answered it truthfully instead.** PRD 7.3 requires "show the meals the answer drew on as citations", and an empty `citedMealIds` passes all four of TSD 5.7's checks vacuously — check 1 asks only whether a cited id is OUTSIDE the prompt's ids. Closed at P24 by a guard in `apps/server/src/routes/chat.ts` that discards such a reply, matching every other containment failure on the route | Reachable against a real model; **unreachable under `AI_FAKE`**, whose echo carries `resolved.citedMealIds` by construction — so nine phases of `AI_FAKE` evidence were consistent with the defect | Low as shipped (the user sees the unavailable copy with a working retry, not a wrong answer) — but the user is denied an answer the domain had already computed correctly | **The alternative is deliberately NOT built, and this row is the record.** `resolved` holds a true statement and its own `citedMealIds`; substituting them would give the user a correct, cited answer instead of a 503, and the construction already exists in the same scope as `AI_FAKE`'s echo. It was not taken because TSD 5.7 specifies discard for an unusable reply and inventing a second degradation shape would be the route deciding product behaviour no document describes. **Two P28 auditors reached OPPOSITE conclusions on the cost, and the disagreement is recorded rather than averaged.** The AI auditor ruled the substitution **cheaper** than this row says: `packages/contracts/src/api.ts:21-26` already admits `{answered: true, source: 'local'}` and `apps/mobile/src/features/assistant/useAssistant.ts:164` never reads `source`, so nothing mechanically blocks it. The contract auditor ruled it **not the better reading**, for a blocker this row did not record. **Both are right about different things** — the schema admits the shape; the SDD's *definition* of that value would be false of it. Which matters more is precisely the call this row exists for. The blocker: `ChatResponse.source` is `'gemma' | 'local'`, and SDD 7.2 defines `'local'` as the domain having answered **without calling the model**. On a substitution path the model *was* called and its words *were* discarded — so **both values are false**, and there is no third value. The alternative therefore needs a **contract amendment**, not a route change, and the same auditor ruled on that basis that it is **not** the better reading. **What the user decides:** whether a contained-out reply should fall back to the domain's statement rather than 503 — and if so, whether `ChatResponse.source` gains a third value naming that case, and whether it applies to all four containment failures or only to a missing citation | AI Engineer | **User decision** |
| R-72 | **All five bottom-tab labels are painted clipped at every phone width, and every box measures as fitting.** `@react-navigation/bottom-tabs` 7.18.18 styles the stacked (beneath-icon) label `{ fontSize: 10 }` with **no `lineHeight`** (`src/views/BottomTabItem.tsx:431-436`), while the beside-icon variant it swaps to at 768 px sets `lineHeight: 24`. react-native-web gives the text element `overflow-y: hidden`, so the glyphs are drawn in a box two pixels shorter than they need and their descenders are cut | **Certain below 768 px**, measured at 320, 375 and 414: five entries, each `scrollHeight 14` against `clientHeight 10` — **the row said 12/10 until P28 re-measured it out of tree; four pixels are lost, not two**. **None at 768** | Low-to-medium — the labels stay legible and the defect is cosmetic, but it is on the one surface visible from every screen in the app, at three of the four supported widths | **Found by a human reading the screenshots `Plan.md` §20 asks for as T-22-07's evidence — no box assertion can see it**, including the one T-22-07 added for this class of defect, because the box and the clip are the same element and the box does fit. That is the argument for the Plan having asked for screenshots. **Two fixes were tried from `TabNavigator.tsx` and both reverted, measured:** `tabBarLabelStyle: { lineHeight: 16 }` made it **worse** (`scrollHeight` 12 → 16 against an unchanged `clientHeight 10`), and adding `height: 16` changed nothing — **this row's stated cause was FALSE, and being false is why both rounds failed.** It said the clipped element was "a different, inner `div`" the prop never reached. It is not: `Label` passes style to a **single** react-native-web `Text`, and attempt one's own `scrollHeight` moving **14 → 16** proves the prop arrived — had it landed elsewhere, nothing would have moved. The real cause is that the label is a **compressed flex item** (`flex: 0 1 auto`), so it was never *unstyled*, it was *squeezed*: the library's own metrics leave `49 − 1 − 10 − 28 = 10` px for a label needing 14, and **no line-box style can win against a flex basis**. Which is why the shipped fix is none of the three remedies this row proposed — it sizes the **bar** (49 → 55) and sets **no label style at all**. So the fix is not a style on that prop: **a third remedy exists and is unmeasured — `tabBarLabelPosition: 'beside-icon'`, which is the variant that already works at 768** — or it needs `tabBarLabel` rendered as this project's own `AppText`, which carries accessible-name consequences, or a library upgrade. Neither is a one-line edit and neither was guessed at inside another phase's round. **Pinned as a CHECKED gap**, not a described one: `e2e/specs/text-clipping.spec.ts` uses `test.fail(width < 768)`, so it turns red the day the labels are fixed **and** the day 768 stops working — a blanket `test.fail` was tried and correctly turned red at 768. A separate sensitivity control with a much larger clip proves the measurement fires, so an expected-to-fail passing for the wrong reason cannot hide | Accessibility Engineer | **CLOSED at P28** |
| R-73 | **Explore can reach only 20 of the 60 meals, and tells a screen-reader user there are 60.** `exploreFilters.ts:33` sets `EXPLORE_PAGE_SIZE = 20` and `exploreFilters.ts:91` takes `queryFrom(search, filters, page = 1)` — **but `useMealSearch.ts:118` never passes the third argument**, so every request is page 1, and `ExploreScreen.tsx` has no `onEndReached` and no page state. So plain browsing tops out at twenty records with no affordance to go further. Separately, `ExploreScreen.tsx:215` announces `accessibilityLabel={`${state.total} meals`}` — **the unfiltered total, 60, over a twenty-row list** | **Certain**, and verified twice: the missing argument and the absent `onEndReached` were each read from the source | **Medium, and two distinct harms.** A browsing user cannot reach two thirds of the catalogue, though search and the filter chips re-query and so surface other records — the meals are not lost, the *browse* path is bounded. The false count is worse in kind than in size: it is a statement to the one user who cannot see the list length for themselves | **Recorded, not fixed — P28 records and remediation is a new decision.** FR-010's "Results page locally" clause therefore has **no implementation and no test**, which is the one PARTIAL in T-28-01's 14/1/0. **And the label's test could not have caught it:** swapping `state.total` for `state.meals.length` reddens **0 of 19**, because `Explore.dom.test.tsx:32`'s stub always sets `total: meals.length` — a fixture drawn from the same shape as the code, which is the failure mode §10.4 names. **What the user decides:** whether Explore gains paging (`onEndReached` plus page state) or whether `EXPLORE_PAGE_SIZE` should simply exceed the catalogue, and separately whether the announced count should be `state.meals.length`. The second is a one-line change and is the half a screen-reader user meets | Mobile Engineer | **User decision** |
| R-74 | **A 101-character search breaks Explore, and the retry it offers fails identically.** `apps/server/src/routes/meals.ts:53` bounds the query at `z.string().trim().min(1).max(100)`, and **`apps/mobile/src/shared/components/SearchField.tsx` sets no `maxLength`** while `exploreFilters.ts`'s `queryFrom` forwards the text unbounded. So 101 pasted characters answer **400 `invalid_request`**, which `useMealSearch.ts:68-73` renders as Explore's **ERROR** state — not "no meals match" — with a retry that repeats the same rejected request | **Certain**, and trivially reachable by pasting. The bound itself is **unpinned**: deleting `.max(100)` reddened **0 of 173**, with a control on the same line reddening 4, so the line executes and nothing asserts it | Low frequency, but the failure is confusing rather than graceful: the user is shown a server error and a button that cannot work, for text they typed | **CLOSED at P28, both doors, and one clause of this row was false.** It said the ceiling was "a product question no document answers": **TSD §5.4 answers it** — `| `query` | string | 1–100 chars |` — so there was no user decision to wait for, only a figure declared three times and agreed with by nobody. (Third register row found carrying false text, after R-44's symptoms and R-55's justification.) **Door 1, the field:** `SearchField.tsx` now sets `maxLength` and slices `onChangeText`, so the 101st keystroke and the over-long paste are both unreachable. **Door 2, the route param:** `ExploreScreen.tsx` seeded `search` from `?query=` with no bound at all — measured `param=101 sent=101`, straight to the 400 — and now truncates at the same ceiling, because a link and a keystroke must produce one screen state, not two. Filed independently by two agents from opposite sides. **The figure is declared once**, `MEAL_QUERY_MAX_LENGTH` in `packages/contracts`, which is R-78's fix applied to R-78's shape: three copies (route, field, document) that could not see each other. **And the bound is pinned now, which was half the original finding:** deleting `.max(100)` used to redden 0 of 173; the route's ceiling is asserted on the wire at N and N+1, and `MAX - 1`, `MAX + 1` and no-bound each redden it with symmetric messages, while removing the seed's slice or narrowing it by one each reddens the Explore spec | Mobile Engineer | **Closed** |
| R-75 | **The denied-claim sweep cannot simply be widened to every user-facing string, because a string PRD FR-007 REQUIRES is flagged by the guard.** **Measured exactly at P28: three required strings, all tripping on the single word `medical`.** `HomeScreen.tsx` ships *"Suggestions are filtered using the allergies you set… This is not medical advice."*, `ExploreScreen.tsx` *"Explore lists the whole catalog… This is not medical advice."*, and `OnboardingScreen.tsx` *"It is not medical advice, and it cannot replace…"* — and `deniedClaimIn` returns `"medical"` for each, because `claimDenylist.ts`'s `PRD_EXTENSIONS` carries `medical` and **negation is deliberately not an exemption** (TSD 5.7 — otherwise *"not unhealthy"* would pass). **The asymmetry is the whole difficulty:** that rule is right for a model *asserting* about a meal and wrong for our own disclaimer *declining to advise*, and a phrase matcher cannot tell "I will not give medical advice" from `claimDenylist.ts`'s own cited example, "Please seek medical advice before ordering". So the exemption has to be keyed on the **whole string**, never on the word. **And the cost is bounded:** apart from `medical`, nothing in mobile copy trips the guard — six apparent hits on `safe` and `treats` are two `react-native-safe-area-context` import specifiers, three docstrings and a test harness, none of them shipped copy. (This row cited `HomeScreen.tsx:130`; the string is at :127 — a line citation into a living file, expired, §6.1q again. Quoted now.) Three further copy sources sit outside both sweeps: `apps/mobile/src/features/settings/settingsCopy.ts` (30 strings), `apps/server/src/errors.ts`'s `MESSAGES` (5), and nine mobile `.tsx` files with inline copy | Certain as a design constraint; **no live defect**, and the "all are clean" measurement this cell used to carry **expired within the same session**: it was taken before `ExploreScreen.tsx`'s disclaimer existed, and that string — added at P28 closing T-24-03 — carries the same flagged sentence. Re-measured: **three flagged, all required, all on `medical`; everything else clean** | The risk is in the *fix*: a naive widening reddens the gate on a requirement-mandated sentence, and the obvious repair — exempting negations — reopens the hole the denylist exists for | **Recorded before anyone tries the obvious thing.** Closing R-70 structurally on both sides needs a **vetted-exemption mechanism**: a short list of strings permitted to contain a denied term, each with a recorded reason and each asserted to **still contain it**, so an exemption that stops being needed fails rather than lingering. **What the user decides:** build that and widen the sweep, or keep the sweeps narrow and accept that copy outside them is unguarded | AI Engineer | **User decision** |
| R-76 | **`build:server` step 3 was never committed, because `.gitignore`'s bare `build/` matched the directory the staging script lived in.** `git check-ignore -v` returned `.gitignore:6:build/  apps/server/build/stagePackages.mjs`; nothing was tracked under that directory; and **`git status` stayed clean**, because an ignored file is not untracked-and-reported — it is invisible. So the commit that closed R-69 carried the fix minus its third step, and on a clean clone `npm run build:server` would fail with a missing file — arguably worse than the unrunnable artefact it replaced, because the documented command breaks outright | **Was certain on any clone**, invisible on this machine where the file exists | **Would have reopened R-69 for every reader**, and `docs/phase-reports/P26.md` asserts R-69 closed, so the phase record was false | **CLOSED on discovery, by removing the collision rather than negating it.** `!apps/server/build/` would have worked and would have left the trap for the next file added under any `build/` directory. The script now lives at `apps/server/scripts/stagePackages.mjs`, matching the root `scripts/dev.mjs` convention, so `build/` keeps meaning build **output**. `.gitignore` carries a comment recording the trap and the ESLint block widened to `**/scripts/*.mjs`. Verified: tracked, not ignored, `build:server` exit 0, artefact boots, `/health` 200 with `mealCount` 60. **The transferable lesson is the check that was missing — the pre-commit sweep looked for files wrongly INCLUDED (secrets, `dist/`, machine paths) and never for one wrongly EXCLUDED. Over-inclusion shows up in `git status`; under-inclusion is silent** | Release Engineer | **Closed at P28** |
| R-77 | **The web export has no `main` landmark on any screen.** Measured in the built export: zero `role="main"` anywhere, against one heading on Home. A screen-reader user has no "skip to content" target and no way to jump past the tab bar and header on every navigation | Certain on the web build; not applicable on native, which has no landmark concept | Medium for a screen-reader user on the delivery surface P22 makes first-class — landmarks are the primary navigation mechanism, and the app offers none | **Recorded, not fixed — P28 records.** `Plan.md` §20 lists `Semantic HTML` as a row and the P28 web audit ruled it **NOT MET** for this reason. One `accessibilityRole="main"` on the screen container inside the navigator would close it, but react-native-web maps a limited role set and the mapping needs verifying in `node_modules` before anyone claims it works — the same discipline that caught three false claims about that library this window. **What the user decides:** whether to add it now or carry it | Accessibility Engineer | **User decision** |
| R-78 | **The denied-claim list exists in TWO copies and they have already diverged by three phrases.** `apps/server/src/ai/claimDenylist.ts` ships **22** — sixteen documented plus six `PRD_EXTENSIONS` added by ruling. `apps/mobile/src/features/assistant/assistantCopy.test.ts` holds its **own transcribed copy** with only **19**, missing `safer`, `safest` and `safely`; its own docstring explains why it is a copy at all — *"`apps/mobile` may not import"* from the server, so `deniedClaimIn` is unreachable and the phrases are transcribed **from TSD §5.7**. **So the mobile copy tracks the DOCUMENT and the server tracks document + rulings, and they drifted the moment a ruling added three phrases** | **Certain and already happened** — the divergence is 3 of 22, measured at P28 | **No live defect:** every mobile copy string was measured clean against the real guard, and the three missing phrases are inflections of `safe`, which is itself on both lists. The risk is that the next ruling widens the gap silently | **This is R-70 one level out, and the same shape as the failure-to-outcome mapping that lived in three copies with two already diverged** — where *"nothing could have caught it: there is no assertion any suite can make about a constant in a file it does not import."* **The fix that makes the divergence unrepresentable is to move the phrases into `packages/contracts`**, which both sides may import — and that is a spine change to a package twenty agents are reading, so it is recorded rather than taken mid-wave. **What the user decides:** move the list to `packages/contracts` (one copy, no drift possible), or keep two copies and add a test that fails when they differ — which is only possible if something can see both, i.e. the same move | AI Engineer | **User decision** |
| R-79 | **After a machine restart the assistant is 503 forever, and no request the app can issue will ever load the model.** `PRD.md` §10.1 says "The first request after a restart may legitimately fail and then recover on its own" — **there is no mechanism behind that sentence.** `apps/server/src/aiLane.ts` aborts at the budget exactly as TSD §5.5 requires, and Ollama **cancels the in-flight load** when the abort arrives, so the load never completes: a page-cache-cold load needs **43 767 ms** against a 30 000 ms chat budget, and `/api/ps` stayed empty across **240 s and three consecutive requests**. `recommendations.ts` carries a *shorter* 12 s budget and cannot load it either. Reproduced deterministically at a 5 s budget | **Certain**, measured on a real `gemma3:4b` (first real-model run in the project's history). Page-cache-WARM does not 503 at all (17.7 s), which is why nine phases of evidence never met it | Severe on the path a user actually takes — a cold boot, then the headline feature, which answers "The assistant is unavailable right now." every time until something else warms the model | **Recorded, not fixed, and the abort is not the defect.** A timeout that did not abort would leak the single flight, so `aiLane.ts` is the site and not the fault. **`Plan.md` §14's R-05 does not cover this**: `AI_KEEP_ALIVE=30m` is about *eviction*, not about a load cancelled before it finishes. **What the user decides:** which remedy — a longer first-request budget, a boot-time warm-up call, or an explicit preload — each of which is product behaviour no document describes | AI Engineer | **User decision** |
| R-80 | **TSD §5.6's `RULES` clause "never write a meal id in prose" is the one clause a model can break with nothing to notice — and `gemma3:4b` broke it.** A real reply carrying `<<<citedMealIds: [english-breakfast, …]>>>` **inside the user-visible answer** passed all four containment checks and would have been rendered: the ids are in `promptMealIds`, the names are permitted, the figures are permitted, and no denied term appears. The `<<<…>>>` wrapper is the shape of `promptSafety.ts`'s `UNTRUSTED_OPEN`, which `neutraliseUntrusted` redacts **inbound** while nothing redacts it outbound | **Certain**, observed in one run of three — same machine, same model, `temperature: 0`, `seed: 7`, byte-identical prompt, which **confirms TSD §5.2's own hedge** that reproducibility is "a debugging property, not a guarantee" rather than contradicting it | Not CRITICAL — nothing false or unsafe is shown, the ids and figures are correct — but a documented requirement is unmet and no guard exists whose loss anyone would notice | **Recorded, not fixed, and both candidate fixes are blocked by the document.** A fifth `ContainmentRule` member is a TSD §5.7 amendment, and **X-49 is already open on exactly that question** ("specifies four checks, the shipped behaviour applies five"). Neutralising the answer outbound is the other candidate and §5.7 forbids it in as many words: a failing reply is "discarded — never repaired, never partially used". **What the user decides:** whether containment gains a fifth rule, or the outbound sentinel is stripped as `promptSafety` already strips it inbound | AI Engineer | **User decision** |
| R-81 | **A whole supported question class is effectively unanswerable, and the branch that does it is correct as specified.** PRD §7.4 supports counting questions and `packages/domain/src/answer.ts` resolves them correctly, but a count carries `namedMeals: []`, so `chatFormat.ts`'s documented no-enum branch leaves the grammar's layer 0 absent and containment check 1 permits **no citation at all**. The model invents one (`MEAL_ID_9`, `MEAL_ID_28`, `7`) and the correct answer is discarded as a 503 | **Certain**, **3 of 3** count questions in **both** measured sessions, so a user asking "how many are vegan?" gets "The assistant is unavailable right now." essentially always | The domain has the right answer and the user never sees it — the worst shape a containment discard can take, and the same shape as R-71 from the other direction | **CLOSED at P28, by the first of the two options, and it needed neither containment change.** The constraint belongs on the **array**, not the item: `maxItems: 0` admits exactly `[]`, which satisfies `required: ['citedMealIds']` and leaves check 1 nothing to reject. The P24 guard does not fire either, because it is keyed on `resolved.namedMeals.length > 0` and a count has none — so the shape it exists to forbid was never this one. **Verified against a real `gemma3:4b`: 9 of 9** count questions answered, every one with zero citations, where 3 of 3 had failed. **This is a TSD §5.5 amendment** — that section specifies `chatFormat` verbatim — made in the permitted direction on the user's explicit instruction: PRD §7.4 lists **Count** as an answerable shape, §5.5's schema made Count unanswerable, and PRD outranks TSD, so the lower document was the defect. The rationale is recorded in TSD §5.5 beside the change. **And the test guarding this branch was prophetic from the wrong side:** its comment warned that `minItems: 1` would 503 every count question, when a *permissive* bound did it instead — a lesson worth keeping, that an over-permissive grammar is as fatal as an over-demanding one when something downstream rejects what it permits | AI Engineer | **Closed** |
| R-82 | **The whole catalogue is invisible at 320 px — `explore-list` renders at height 0 with `aria-label="20 meals"`.** `explore-screen` is **513 px** at 320×568 (568 less the 55 px bar) and its three `flex-grow: 0` siblings already exceed it: the search field **50**, `explore-filters` **328**, `explore-disclaimer` **258** — **636 in 513**. The list is the only growing child, so it gets what is left, and what is left is nothing: present, populated, announced, invisible | **Certain**, measured element by element at 320×568, and **320 alone** — 375 and 414 both pass, because the chips wrap onto fewer rows there. `Plan.md` §20 requires "every screen usable at 320 px" | **High at the width it affects**: Explore is the catalogue, and at the narrowest supported viewport there is nothing to browse. It also shut the door `viewport.spec.ts` used to reach `meal-details-screen`, so three other screens went unmeasured at 320 px | **Half fixed, half a design decision, and this wave caused it.** The list now carries `flex: 1` (a `0%` basis rather than react-native-web's content basis) — necessary, measured to be genuinely applied, and **insufficient**: a first version of that comment claimed the basis WAS the cause and the measurement disproved it. The 258 px notice is new this wave, added closing T-24-03, which is why a clean clone of `dc4eca5` was green at 276 — **a correct fix for one requirement broke another**, the P16–P18 CRITICAL's shape exactly. Pinned as a checked gap: `viewport.spec.ts`'s R-82 test asserts the list's height in pixels and is `test.fail` below 375, so it reddens the day the layout is fixed. **What the user decides:** how much of a 568 px screen a safety notice and three chip groups may claim. The notice cannot simply become a `ListHeaderComponent` — it is required in every state, including before the first response, where there is no list to hang a header on | Mobile Engineer | **User decision** |
| R-83 | **The tab bar cannot grow for text a browser enlarges, and `isLargeText` is dead code on the web.** `apps/mobile/src/navigation/TabNavigator.tsx` sizes the bar from `typography.label.lineHeight`, which is `16 × fontScale` — and **react-native-web 0.21.2 hardcodes `fontScale: 1`** while `Dimensions.set` throws in the browser, so `ThemeProvider.tsx` resolves 1 for every web caller. Under Chromium's `--blink-settings=minimumFontSize=32`, which **does** reach px text (10 → 32), the live tab label measures `needed 43 / given 16` with the bar still at 54 px | **Certain**, measured live, and it bounds R-72's fix rather than contradicting it: **native yields 32 and fits**, so the fix is sound exactly where the scale is visible | Medium, and web-only: a user who enlarges text in the browser gets clipped tab labels that the app has no way to learn about | **Recorded, not fixed — the platform is the constraint.** A bar sized from a scale the platform refuses to report cannot respond to it; the alternatives are measuring rendered text (a layout pass the library does not expose) or a CSS-level media query outside the token system. **This is also the ceiling on T-23-03 for the web**, which is why that row stays Partial: `e2e/specs/font-scaling.spec.ts` raises the scale by multiplying each screen's own font triplet, which is a faithful simulation and **not an OS setting** | Mobile Engineer | **User decision** |
| R-84 | **Every unmatched route answers *"That meal could not be found."*, including `GET /`.** Measured across seven paths — `/`, `/nonsense`, `/api`, `/api/v1`, `/api/v1/nonsense`, `/api/v1/mealsXYZ`, `/api/v1/meals/a/b` — all **404 `meal_not_found`** on a server whose root names no meal. **And the handler's two branches were byte-identical:** an `if (request.path.startsWith('/api/v1/meals/'))` arm built the same error as the fall-through, under a comment explaining at length why "the trailing slash matters" | **Certain**, found by the user opening `localhost:4000` by hand — which is worth recording: nine phases of automated evidence never looked at the API root, because no test and no client requests a path the contract does not define. **Deleting the dead branch failed 0 of 2617 tests** | **MINOR, and bounded by who can see it.** `routes.ts` only ever calls paths that exist, so this text reaches someone poking the API directly and never a user reading a screen — had it been reachable from the UI it would be a false statement in the interface and a different severity | **Half fixed.** The dead branch is gone and its comment rewritten to describe what the code does, keeping the history because the branch was trying to avoid exactly the defect that remains (F-6's shape: an explanation outliving its behaviour). **The message is NOT fixed and cannot be without an amendment:** TSD §3.5 fixes the wire codes at **five** and its table maps 404 to `meal_not_found` alone, so no code says "that route does not exist"; a sixth (`not_found`) was tried once and reverted because it carried no `retryable` while `ApiErrorBody` requires one. **What the user decides:** add a sixth code with a `retryable` (a TSD §3.5 amendment), or answer a non-API path with a bare 404 and no envelope on the ground that §3.5's table is the contract for `/api/v1/*` and says nothing about `/` | Backend Engineer | **User decision** |
| R-85 | **The assistant's own guidance copy told users to ask three questions it then refused.** `chatCopy.ts`'s `noInformation` offered *"how they rank by calories or protein"*, *"what they come to altogether"*, and `greeting` additionally offered *"which has the most protein"* — measured against the live assistant, **three of the six suggested forms refused** | **Certain**, found by the user asking two ordinary questions and reading the reply. Two independent causes: the four nutrition fields are `null` for **53 of the 60** seeded meals and TSD §4.9's `gather` refuses a field any candidate cannot supply (`field-partially-known`), while *"what they come to altogether"* names a shape and no field, which §4.9's matching rule 5 makes `incomplete-intent` **by contract** | **High for trust rather than for safety.** A bounded assistant that misstates its bounds reads as broken, where one that states them narrowly reads as merely limited — and this copy exists precisely to say what the bounds are | **FIXED.** Both strings now name only forms measured to resolve — cheapest, quickest, longest to make, total price, ranking by price or preparation time, how many, and what options — **9 of 9 verified** after the change. Nutrition is not named again until the catalog-coverage pass makes it true, which is R-03. The resolver was deliberately NOT changed to default a field-less `total`: §4.9 rule 5 makes that `incomplete-intent`, so the copy was the defect and the resolver was not | Backend Engineer | **Closed at P28** |
| R-86 | **The model paraphrases the domain's sentence instead of restating it, and the paraphrase leaks the prompt's own section label into user-visible text.** The domain resolves *"The meals shown come to $50.50 in total."*; the user reads **"The total price of the MEALS listed is $50.50."** — `MEALS` being TSD §5.6's prompt section name. TSD §5.6's `RULES` clause 1 says *"restate the ANSWER sentence and do not check or change it"*, and **nothing in containment checks that the restatement happened** | **Certain and highly reproducible: 5 of 5** on "what is the total price", against `temperature: 0` and `seed: 7`. All four checks pass legitimately — the citation is permitted, no denied term appears, `$50.50` is a permitted figure, and no meal name is invented | Moderate: nothing false or unsafe is shown and the figure is right, but the sentence is not the one the domain guaranteed, and prompt scaffolding in user copy reads as a bug to anyone who sees it | **Recorded, not fixed — same wall as R-80.** This is R-80's family and **more reproducible than R-80 itself** (5 of 5 against 1 of 3): both are the model emitting prompt structure that containment has no check for. A fifth `ContainmentRule` is a TSD §5.7 amendment (**X-49** is already open on the count), and §5.7 forbids repairing a reply — *"discarded — never repaired"*. **What the user decides:** a containment check that the answer contains the resolved statement (strict, and it would discard every paraphrase including harmless ones), a `RULES` clause hardening the restatement instruction, or returning `resolved.statement` verbatim and using the model only where it adds something | AI Engineer | **User decision** |
...
| R-20 | A criterion listing resolves over `context` per TSD §4.9, so it can omit an eligible meal that did not rank in the top five. "What vegetarian options do I have?" lists the vegetarian meals among the five retrieved, not all of them | High | **This cell said "Low — the sentence is true, merely incomplete", and that was measured wrong at P28: with a PERIOD criterion it refuses outright.** A dinner listing answered *"I do not have that information"* while **38** dinner meals were eligible, because none of the five ranked meals happened to suit dinner. Incomplete is survivable; a false refusal on the question this app is named after is not | **CLOSED at P28, by both halves this row proposed.** A criterion listing now resolves over `eligible`, and because `eligible` is unbounded it **caps what it names** at five and **states both numbers** — "Here are 5 of your 38 dinner meals" — which is this row's own "wording makes the number visibly a count of what is shown rather than a total". Both figures are permitted, or check 3 would discard a sentence the domain composed itself. **It is the TSD §4.9 scope-table amendment this row said was required**, made on the user's explicit instruction and in the permitted direction: PRD §7.4's **Listing** shape already covers a criterion-scoped list, and `count` already scoped to `eligible` and answered the same question correctly — so the amendment makes the table consistent with itself | Domain Engineer | **Closed** |
