...
| R-75 | **The denied-claim sweep cannot simply be widened to every user-facing string, because a string PRD FR-007 REQUIRES is flagged by the guard.** `apps/mobile/src/features/home/HomeScreen.tsx:130` ships *"…This is not medical advice."* and `deniedClaimIn` flags it, because `claimDenylist.ts` carries `medical`/`medically` and **negation is deliberately not an exemption** (TSD 5.7 — otherwise *"not unhealthy"* would pass). Three further copy sources sit outside both sweeps: `apps/mobile/src/features/settings/settingsCopy.ts` (30 strings), `apps/server/src/errors.ts`'s `MESSAGES` (5), and nine mobile `.tsx` files with inline copy | Certain as a design constraint; **no live defect** — every string outside the two sweeps was measured against the real `deniedClaimIn` and all are clean | The risk is in the *fix*: a naive widening reddens the gate on a requirement-mandated sentence, and the obvious repair — exempting negations — reopens the hole the denylist exists for | **Recorded before anyone tries the obvious thing.** Closing R-70 structurally on both sides needs a **vetted-exemption mechanism**: a short list of strings permitted to contain a denied term, each with a recorded reason and each asserted to **still contain it**, so an exemption that stops being needed fails rather than lingering. **What the user decides:** build that and widen the sweep, or keep the sweeps narrow and accept that copy outside them is unguarded | AI Engineer | **User decision** |
