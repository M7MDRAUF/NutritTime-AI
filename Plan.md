...
SQG-09 caps a new file at 350 lines. **Forty-four files exceed it.** Each is approved here with its
...
| SQG-09 | Every new **file** ≤ 350 lines — PRD §10.4 says files, not only components; an exception is documented and approved. **Forty-four approved exceptions are recorded in §17.1 below**; every one is a single cohesive table or one exhaustive `switch`, where splitting would scatter a thing a reviewer needs to read whole |
